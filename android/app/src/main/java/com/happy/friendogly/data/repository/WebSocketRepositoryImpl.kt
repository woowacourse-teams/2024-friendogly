...
class WebSocketRepositoryImpl(private val source:WebSocketDataSource):WebSocketRepository {
    ...
    override suspend fun publishSend(chatRoomId: Long, content: String) = source.publishSend(chatRoomId, content)

    override suspend fun publishLeave(chatRoomId: Long) = source.publishLeave(chatRoomId)

    override suspend fun subscribeMessage(chatRoomId: Long): Flow<ChatComponent> = source.subscribeMessage(chatRoomId).map {
        when(it.messageType){
