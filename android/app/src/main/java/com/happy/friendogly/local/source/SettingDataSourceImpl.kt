...
class SettingDataSourceImpl(
    ...
    override suspend fun saveAlarm(isSet: Boolean): Result<Unit> = runCatching {
        alarmModule.saveSetting(isSet)
    }

    override suspend fun getAlarm(): Result<Boolean> = runCatching {
        alarmModule.isSet.first()
    }

    override suspend fun deleteAlarm(): Result<Unit> = runCatching {
